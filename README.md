...
Project ini merupakan tahap awal dari alur *Sistem Machine Learning (SML)*: eksplorasi data mentah, penentuan langkah-langkah preprocessing, lalu pembuatan script otomatisasi (`automate_model.py`) yang menghasilkan dataset bersih dan siap dipakai untuk pelatihan model.

Alur kerja repo ini:

1. Data mentah (`ai4i2020.csv`) dieksplorasi dan diproses secara manual pada notebook eksperimen.
2. Langkah-langkah preprocessing yang terbukti berhasil dituangkan ke dalam script Python (`automate_model.py`).
...
└── preprocessing/
    ├── model.ipynb        # Notebook eksperimen (EDA & eksplorasi preprocessing)
    └── automate_model.py             # Script otomatisasi preprocessing
```

## Dataset

**AI4I 2020 Predictive Maintenance Dataset** dari UCI Machine Learning Repository yg berisi data sensor mesin (10.000 baris) yang digunakan untuk memprediksi kegagalan mesin.
...
## Tahapan Preprocessing (`automate_model.py`)
...
- Ada perubahan pada `ai4i2020_raw/**` atau `preprocessing/automate_model.py` di branch `main`, atau
...
1. **Clone repository**
   ```sh
   git clone https://github.com/alfdmsr/predictive-maintenance-preprocessing.git
   cd predictive-maintenance-preprocessing
   ```
...
3. **Jalankan script preprocessing**
   ```sh
   python preprocessing/automate_model.py
   ```
...
4. **(Opsional) Jalankan notebook eksperimen** untuk melihat proses eksplorasi data secara lengkap:
   ```sh
   jupyter notebook preprocessing/model.ipynb
   ```
